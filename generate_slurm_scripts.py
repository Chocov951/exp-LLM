...
python -u all_in_one_rankllm.py --dataset {dataset} --bm25_topk {bm25_topk} --filter_topk {reject_number} --load_filtered_rundict "rankllm_results/rundicts/filtered_{dataset}_{filter_method}_filter{rej}_top{bm25_topk}.json" --listwise_model "custom_llm" --custom_model "${{MODEL_SHORT}}" --ranking_method listwise --stage rerank 
            ...
            for model_name in model_names:
                for filter_method in filter_methods:
                    ...
                    script_content = slurm_template.format(
                        model=model_name, dataset=dataset, bm25_topk=bm, reject_number=rj, filter_method=filter_method, rej=rej)
                    script_filename = f"slurm_{dataset}_{model_name}_{filter_method}_filter{rej}_top{bm}.slurm"
