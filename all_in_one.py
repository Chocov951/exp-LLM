...
def get_args():
    ...
    parser.add_argument("--model_name", help="model to use", default="mistral",
                        choices=["qwen3", "qwen14", "qwen32", "qwen72", "calme", "llama", "mistral"], type=str)
    ...
    parser.add_argument("--reject_number", type=int, default=10, help="Number of passages to keep in the reject phase")
...
if __name__ == '__main__':
    ...
    # Reject
    emissions_file_name = f"emissions_reject_{args.dataset}_{args.model_name}_reject{args.reject_number}_topk{args.bm25_topk}_cache.csv"
    tracker_reject = OfflineEmissionsTracker(country_iso_code="FRA", tracking_mode="process", on_csv_write="append",
                                      project_name="LLM-Rerank", log_level="warning", output_dir=emissions_folder, output_file=emissions_file_name)
    # Rerank
    emissions_file_name = f"emissions_rerank_{args.dataset}_{args.model_name}_reject{args.reject_number}_topk{args.bm25_topk}_cache.csv"
    ...
    def generate_responses(input, passages, corpus_texts):
        ...
        try:
            ...
            start_time = time.time()
            outputs = pipe(messages, max_new_tokens=10*args.reject_number+10, num_return_sequences=1, do_sample=False)
    ...
    if os.path.exists(f'rundicts/rundict_{args.dataset}_{args.model_name}_rerank{args.reject_number}_topk{args.bm25_topk}_cache.json'):
        with open(f'rundicts/rundict_{args.dataset}_{args.model_name}_rerank{args.reject_number}_topk{args.bm25_topk}_cache.json', 'r', encoding='utf-8') as f:
            rundict_rerank = json.load(f)
        with open(f'rundicts/rundict_{args.dataset}_{args.model_name}_reject{args.reject_number}_topk{args.bm25_topk}_cache.json', 'r', encoding='utf-8') as f:
            rundict = json.load(f)
            print("\nRerank rundict loaded\n")
    else:
        # Process responses and evaluate
        if os.path.exists(f'rundicts/rundict_{args.dataset}_{args.model_name}_reject{args.reject_number}_topk{args.bm25_topk}_cache.json'):
            with open(f'rundicts/rundict_{args.dataset}_{args.model_name}_reject{args.reject_number}_topk{args.bm25_topk}_cache.json', 'r', encoding='utf-8') as f:
                rundict = json.load(f)
                print("\nReject rundict loaded\n")
        else:
            rundict = {}
            queries_in_qrels = [query_id for query_id in qrels_test.keys()]
            tracker_reject.start()
            for tracker_index, qkey in tqdm(enumerate(queries_in_qrels)):
                ...
                top_k_passages = [corpus_test[doc_id] for doc_id in top_k_passages_ids]

                corpus_batches, biggest_output = split_queries_by_max_length(top_k_passages, max_length, len(qtext), prompts)
                nb_batches = len(corpus_batches)
                print(f"Nombre de batchs créés : {nb_batches}")
                print(f"Taille moyenne des batchs : {sum(len(batch) for batch in corpus_batches)/nb_batches:.2f} passages")
                for i, passages in enumerate(corpus_batches):
                ...
                for index, score in responses.items():
                    did = corpus_ids[index]
                    if qid not in rundict:
                        rundict[qid] = {}
                    if did not in rundict[qid]:
                        rundict[qid][did] = score
                    else:
                        rundict[qid][did] += score
            tracker_reject.stop()
            # Save reject rundict
            with open(f'rundicts/rundict_{args.dataset}_{args.model_name}_reject{args.reject_number}_topk{args.bm25_topk}_cache.json', 'w', encoding='utf-8') as f:
                json.dump(rundict, f, indent=4)

            rundict_rerank = {}
            tracker_rerank.start()
            for tracker_index, item in tqdm(enumerate(rundict.items())):
                qid, docs = item
                try:
                    corpus_test = queries_test[qid]['passages']
                    documents = {doc: corpus_test[doc] for doc in docs.keys()}
                except:
                    ...
                    try:
                        ...
                        print(f"Error with query {qid}")
                        continue
                if len(documents) != 0:
                    response = generate_rerank(queries_test[qid]['query'], documents)
                    gen_rerank_count += 1
                    rundict_rerank[qid] = response
                else:
                    print(f"Documents are empty for query {qid}")
                    rundict_rerank[qid] = {}
            tracker_rerank.stop()

        # Save rerank rundict
        with open(f'rundicts/rundict_{args.dataset}_{args.model_name}_rerank{args.reject_number}_topk{args.bm25_topk}_cache.json', 'w', encoding='utf-8') as f:
    ...
    with open(f'emissions/emissions_reject_{args.dataset}_{args.model_name}_reject{args.reject_number}_topk{args.bm25_topk}_cache.csv', 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f, delimiter=',')
        tracker_reject = next(reader)
        tracker_reject = {key: value for key, value in tracker_reject.items()}
    with open(f'emissions/emissions_rerank_{args.dataset}_{args.model_name}_reject{args.reject_number}_topk{args.bm25_topk}_cache.csv', 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f, delimiter=',')
    ...
    with open(f'paper_res/results_{args.dataset}_{args.model_name}_reject{args.reject_number}_topk{args.bm25_topk}_cache.json', 'w', encoding='utf-8') as f:
