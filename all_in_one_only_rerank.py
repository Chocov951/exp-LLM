...
if __name__ == '__main__':
    ...
    # Rerank
    emissions_file_name = f"emissions_rerank_{args.dataset}_{args.model_name}_rerank_all_once_topk{args.bm25_topk}.csv"
    ...
    def generate_rerank(query, docs):
        ...
        try:
            ...
            start_time = time.time()
            outputs = pipe(messages, max_new_tokens=5*args.bm25_topk+10, num_return_sequences=1, do_sample=False)
        ...
        for i, id in enumerate(llm_rep):
            ...
            try:
                doc_id = int(id)
                if str(doc_id) not in responses:
    ...
    if os.path.exists(f'rundicts/rundict_{args.dataset}_{args.model_name}_rerank_all_once_topk{args.bm25_topk}.json'):
        with open(f'rundicts/rundict_{args.dataset}_{args.model_name}_rerank_all_once_topk{args.bm25_topk}.json', 'r', encoding='utf-8') as f:
            rundict = json.load(f)
        ...
        for tracker_index, qkey in tqdm(enumerate(queries_in_qrels)):
            ...
            passages = {doc_id: corpus_test[doc_id] for doc_id in reranked_passages_ids}
            simple_id_passages = {str(i): passage for i, passage in enumerate(passages.values())}
            id_matches = {str(i): doc_id for i, doc_id in enumerate(reranked_passages_ids)}
            responses = generate_rerank(qtext, simple_id_passages)
            new_responses = [id_matches[r] for r in responses.keys() if r in id_matches]
            reranked_passages_ids = [r for r in new_responses if r in reranked_passages_ids]
            gen_rerank_count += 1
            print("--------------------")
        ...
        rundict_rerank = rundict.copy()
        # Save rerank rundict
        with open(f'rundicts/rundict_{args.dataset}_{args.model_name}_rerank_all_once_topk{args.bm25_topk}.json', 'w', encoding='utf-8') as f:
    ...
    with open(f'emissions/emissions_rerank_{args.dataset}_{args.model_name}_rerank_all_once_topk{args.bm25_topk}.csv', 'r', encoding='utf-8') as f:
    ...
    with open(f'paper_res/results_{args.dataset}_{args.model_name}_rerank_all_once_topk{args.bm25_topk}.json', 'w', encoding='utf-8') as f:
